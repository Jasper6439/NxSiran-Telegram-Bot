...
Telegram Bot 子包 — 命令、消息处理器、Web API、桥接、分析、导入器。
...
| `web/` | HTTP API 路由 (aiohttp), 页面服务, CORS | 10 |
| `bridge/` | VM 远程命令/文件桥接, MiniApp 消息同步 | 2 |
| `analysis/` | 聊天记录解析与统计分析 | 1 |
| `importers/` | 外部数据导入（微信聊天记录/视频） | 2 |
...
| `misc.py` | 杂项命令聚合（quota, anniversary, summarize, research, relay, updater） |
| `quota.py` | 额度查询与重置 |
| `summarize.py` | URL 摘要生成 |
| `updater.py` | Bot 版本检查与更新 |
| `anniversary_cmd.py` | 纪念日管理命令 |
| `research.py` | Gemini 深度研究 |
| `relay.py` | Telegram 消息历史搜索 |
| `import_cmds.py` | 聊天记录导入命令 |
...
| `auth_routes.py` | 用户注册/登录/找回密码 API |
...
| `skills_state.py` | Skills 持久化状态管理 |
| `sync_routes.py` | Web <-> Telegram 双向消息同步 API |
| `mobile_routes.py` | 移动端触控 API（虚拟方向键/点击交互/滑动手势/配置） |
