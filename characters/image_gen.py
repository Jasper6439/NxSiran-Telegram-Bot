...
from system.config import CHAT_TOPIC_THREAD_ID, (
...
async def send_selfie_to_chat(bot, chat_id, caption=None):
    try:
        ...
        if saved:
            ...
            await bot.send_photo(chat_id=chat_id, photo=io.BytesIO(photo_bytes), caption=caption, message_thread_id=CHAT_TOPIC_THREAD_ID)
            ...
            await bot.send_photo(chat_id=chat_id, photo=photo_url, caption=caption, message_thread_id=CHAT_TOPIC_THREAD_ID)
...
async def send_lora_selfie_to_chat(
    ...
    try:
        ...
        try:
            await bot.send_message(chat_id=chat_id, text=loading_msg, parse_mode="Markdown", message_thread_id=CHAT_TOPIC_THREAD_ID)
        except Exception:
            await bot.send_message(chat_id=chat_id, text=loading_msg.replace("*", ""), message_thread_id=CHAT_TOPIC_THREAD_ID)
        ...
        if image_path and os.path.exists(image_path):
            ...
            photo_bytes = await asyncio.to_thread(_read_file_bytes, image_path)
            await bot.send_photo(chat_id=chat_id, photo=io.BytesIO(photo_bytes), caption=caption)
    ...
    try:
        await bot.send_message(chat_id=chat_id, text=error_msg, message_thread_id=CHAT_TOPIC_THREAD_ID)
    except Exception:
        pass
...
async def generate_face_image(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if result.get("success"):
            ...
            photo_bytes = await asyncio.to_thread(_read_file_bytes, filepath)
            await update.message.reply_photo(photo=io.BytesIO(photo_bytes), caption=f"✨ AI生成完成\n场景：{description}")
